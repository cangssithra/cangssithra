# 👋 Assalamualaikum! Hi there!

I'm an **Android Developer** from Malaysia 🇲🇾, passionate about building practical mobile solutions.

## 🚀 What I Do

- 📱 Android & Flutter Development
- 🐄 Agricultural Technology (AgriTech) Solutions
- 🔥 Firebase Backend Integration
- 📊 Data-Driven Mobile Applications

## 🛠️ Tech Stack

**Mobile Development:**
- Android Studio & Flutter
- Dart & Kotlin
- Firebase (Firestore, Auth, Storage)

**Development Practices:**
- Git Version Control
- Modular Architecture
- Clean Code Principles
- Edge Case Handling

---

## 📱 Published Apps

<div align="center">

### 🎮 MTG Damage Calculator
**Magic: The Gathering Combat Calculator**

[![Google Play](https://img.shields.io/badge/Google_Play-50%2B_Downloads-success?style=for-the-badge&logo=google-play&logoColor=white)](https://play.google.com/store/apps/details?id=com.eopticz.mtgdamagecalc)
[![Web App](https://img.shields.io/badge/Try_Web_Version-Live-blue?style=for-the-badge&logo=firebase)](https://mtg-combat-calculator.web.app)

![MTG Calculator](https://img.shields.io/badge/Version-6.0-brightgreen?style=flat-square)
![Rating](https://img.shields.io/badge/Age_Rating-3%2B-yellow?style=flat-square)

</div>

**A powerful tool for Magic: The Gathering players**

✨ **Features:**
- ⚔️ Calculate combat damage for multiple attackers and blockers
- 🎯 Support for MTG keyword abilities (First Strike, Trample, Flying, Deathtouch, etc.)
- 🔍 Search cards from preset library or enter stats manually
- 📊 Clear tabbed interface for easy navigation
- ⚡ Fast and accurate calculations

**Built with:** Flutter • Dart • Firebase • Material Design

**Platforms:** Android • Web

[📥 Download on Google Play](https://play.google.com/store/apps/details?id=com.eopticz.mtgdamagecalc) • [🌐 Try Web Version](https://mtg-combat-calculator.web.app) • [📂 View Source](https://github.com/cangssithra/mtg-damage-calculator)

---

## 🌱 Currently Working On

**Private AgriTech Projects:**
- 🐄 Cattle Management Systems with QR/RFID tracking
- 📊 Livestock Calculation & Analytics Apps
- 🔥 Real-time Data Synchronization
- 📱 Enterprise Mobile Solutions

## 💭 Development Philosophy

> *"Coding is thinking, not typing"*

I focus on:
- ✅ Readable, maintainable code
- ✅ Modular and scalable architecture
- ✅ Proper edge case handling
- ✅ Security best practices
- ✅ Real-world problem solving

## 📫 Let's Connect

From game calculators to agricultural tech - building solutions for real-world problems, one commit at a time 💻

**Developer Name:** MuddybanksKL  
**Email:** electro.opticz@gmail.com

---

![Profile Views](https://komarev.com/ghpvc/?username=cangssithra&color=blueviolet)
